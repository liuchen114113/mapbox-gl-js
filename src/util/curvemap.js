...
const help = 'See https://www.curvemap.com/api-documentation/#access-tokens';
...
function makeAPIURL(urlObject: UrlObject, accessToken: string | null | void): string {
    ...
    if (!accessToken)
        throw new Error(`An API access token is required to use Curvemap GL. ${help}`);
    if (accessToken[0] === 's')
        throw new Error(`Use a public access token (pk.*) with Curvemap GL, not a secret access token (sk.*). ${help}`);

    urlObject.params.push(`access_token=${accessToken}`);
    return formatUrl(urlObject);
}

function isCurvemapURL(url: string) {
    return url.indexOf('curvemap:') === 0;
}

export { isCurvemapURL };

export const normalizeStyleURL = function(url: string, accessToken?: string): string {
    if (!isCurvemapURL(url)) return url;
...
export const normalizeTileURL = function(tileURL: string, sourceURL?: ?string, tileSize?: ?number): string {
    if (!sourceURL || !isCurvemapURL(sourceURL)) return tileURL;

    const urlObject = parseUrl(tileURL);

    // The v4 curvemap tile API supports 512x512 image tiles only when @2x
    // is appended to the tile URL. If `tileSize: 512` is specified for
    // a Curvemap raster source force the @2x suffix even if a non hidpi device.
