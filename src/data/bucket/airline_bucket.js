 ...
 * @Date: 2018-06-13 15:11:38
 * @Last Modified by: liuchen
 * @Last Modified time: 2018-07-05 18:34:08
 */
// @flow

import { AirlineLayoutArray } from '../array_types';

import { members as layoutAttributes } from './airline_attributes';
...
function addLineVertex(layoutVertexBuffer, point: Point, extrude: Point, round: boolean, up: boolean, dir: number, linesofar: number) {
    layoutVertexBuffer.emplaceBack(
        ...
        (linesofar * LINE_DISTANCE_SCALE) >> 6, h_seq, seq, _count);
}
 ...
 * @private
 */
class AirLineBucket implements Bucket {
    distance: number;
    e1: number;
    e2: number;
    e3: number;
    ...
    constructor(options: BucketParameters<LineStyleLayer>) {
        ...
        this.layoutVertexArray = new AirlineLayoutArray();
    ...
    addFeature(feature: VectorTileFeature, geometry: Array<Array<Point>>, index: number) {
        ...
        const join = layout.get('airline-join').evaluate(feature,{});
        const cap = layout.get('airline-cap');
        const miterLimit = layout.get('airline-miter-limit');
        const roundLimit = layout.get('airline-round-limit');

        seq = feature.properties.link_seq;
        _count = feature.properties.total;
    ...
    addLine(vertices: Array<Point>, feature: VectorTileFeature, join: string, cap: string, miterLimit: number, roundLimit: number, index: number) {
        ...
        for (let i = first; i < len; i++) {
            ...
            h_seq = i === 0 ? 10 * feature.properties.start_height : 10 * feature.properties.end_height;//lc添加
            ...
            if (isSharpCorner && i > first) {
                ...
                if (prevSegmentLength > 2 * sharpCornerOffset) {
                    ...
                    this.addCurrentVertex(newPrevVertex, this.distance, prevNormal.mult(1), 0, 0, false, segment);//lc修改addCurrentVertex参数数量减少
        ...
        this.programConfigurations.populatePaintArrays(this.layoutVertexArray.length, feature, index: number);
    }
...
register('AirLineBucket', AirLineBucket, { omit: ['layers'] });

export default AirLineBucket;
