 ...
 * @Last Modified time: 2018-07-06 16:19:31
 */
// @flow

import { DynamicLineLayoutArray } from '../array_types';

import { members as layoutAttributes } from './dynamic_line_attributes';
...
import type LineStyleLayer from '../../style/style_layer/dynamicLine_style_layer';
 ...
 * @private
 */
class DynamicLineBucket implements Bucket {
    distance: number;
    e1: number;
    e2: number;
    e3: number;
    ...
    constructor(options: BucketParameters<LineStyleLayer>) {
        ...
        this.layoutVertexArray = new DynamicLineLayoutArray();
    ...
    addFeature(feature: VectorTileFeature, geometry: Array<Array<Point>>, index: number) {
        ...
        const join = layout.get('dynamicLine-join').evaluate(feature,{});
        const cap = layout.get('dynamicLine-cap');
        const miterLimit = layout.get('dynamicLine-miter-limit');
        const roundLimit = layout.get('dynamicLine-round-limit');
        seq = feature.properties.link_seq;
        total = feature.properties.total_link_count;
        offset = feature.properties.offset_link_count;
...
register('DynamicLineBucket', DynamicLineBucket, { omit: ['layers'] });

export default DynamicLineBucket;
